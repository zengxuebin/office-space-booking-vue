...
import { getCreditScorePage } from '@/api/creditScore'

const levels = ref<any[]>([])
const users = ref<any[]>([])

onMounted(() => {
  getDictOption('biz_credit_level').then(res => {
...
watchEffect(() => {
  if (users.value.length) {
    ...
    formConfig.items[0].itemRender.options = users.value
  }
  if (levels.value.length) {
    const { formConfig } = gridOptions
    formConfig.items[1].itemRender.options = levels.value
  }
})
...
const gridOptions = reactive<VxeGridProps>({
    ...
    ajax: {
      ...
      query: ({ page, sorts, filters, form }) => {
        return new Promise(resolve => {
          ...
          // 调用方法
          getCreditScorePage(data).then(res => {
            const data = res.data
            resolve({
  ...
  columns: [
    ...
    },
    {
      ...
      align: "center",
      formatter: ({ cellValue }) => {
        return convertDict(levels.value, cellValue)
      }
    },
    {
      field: 'createTime',
